...
void compile_cpp_header(string submission_id){	
	string bash = "g++ ./workspace/main_" + submission_id + ".cpp func.cpp -o ./workspace/main_" + submission_id;
	system(bash.c_str());
}


void compile_cpp(string submission_id){
	string bash = "g++ ./workspace/main_" + submission_id + ".cpp -o ./workspace/main_" + submission_id;
	system(bash.c_str());	
}
...
string shtp_cpp_cli_run(string submission_id, string test){
	string workspace_path = "/home/iko/Desktop/proj/Checker/Checker_2/workspace/";
	string mount_to = "/home/code";
	string image = "ubuntu:latest";
	string entrypoint = "bash -c \"./home/code/main_" + submission_id + " <<< '" + test + "'\"";
	string bash = "sudo docker run --network none -itd -v " + workspace_path + ":" + mount_to + " " + image + " " + entrypoint;
...
json test_one_func(json tests){
	...
	ofstream main_file("./workspace/main_" + submission_id + ".cpp");
...
json test_main(json tests){
	string submission_id = to_string(tests["submit_id"]);
	string bash = "cp main.cpp ./workspace/main_" + submission_id + ".cpp";
	system(bash.c_str());
...
void init_workspace(){
	string bash = "mkdir ./workspace/";
	system(bash.c_str());
}


void clean_workspace(){
	string bash = "rm -r ./workspace/";
	system(bash.c_str());
}
...
// add changable source dir support
...
//	for (auto& el : test_func_ex.items()) {   std::cout << el.key() << '\n'; }
//	test_one_func(test_func_ex, "666", "sum");
//	cout << generate_test_main(test_func_ex, "sum");
	return 0;
}
