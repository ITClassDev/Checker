...
//	tmp solution - compile test statically link stl libs
	bash += "-o " + workspace_path + submission_id + "/main -static";
...
string shtp_cpp_cli_run(string submission_id, string test, json env){
	int time_limit = 1;
	int mem_limit = 6;
	int proc_limit = 2;
...
json test_one_func(json tests){
	...
	for(json::iterator test_it = tests["tests"].begin(); test_it != tests["tests"].end(); ++test_it){
		...
		string submission_container = shtp_cpp_cli_run(submission_id, test["input"], tests["env"]);
...
void clean_git(string git_path){
	string bash = "rm -r " + git_path;
	system(bash.c_str());
}
...
int setup_workspace(json input_json){
	...
	for(const auto &file : filesystem::directory_iterator(git_path)){
		...
		if(file_ext == "cpp" || file_ext == "h" || file_ext == "hpp"){
			if(tests.is_array()){
				for(json::iterator test_it = tests.begin(); test_it != tests.end(); ++test_it){
					json test = *test_it;
					string submission_id = to_string(test["submit_id"]);
...
// {"github_link", "https://github.com/ITClassDev/Checker.git"},
// {"tests_description",{
// 	{{"name","sum"},{"tests",{
// 		{{"input", "100\n120"}, {"output", "220"}},
...
// 		{{"input", "1\n2\n3"}, {"output", "6\n1"}},
// 		{{"input", "60\n30\n15"}, {"output", "105\n1"}}	}},
// 	{"submit_id", 120}, {"env", {{"time", 2},{"mem", 10},{"proc",1}}}
// }} };
...
// 	{"submit_id", 150}, {"env", {{"time", 2}, {"mem", 10},{"proc",1}}}
// }} };



// firstly run: systemctl start docker
// to compile and run this shit: g++ main_gen.cpp docker.cpp types.cpp -lcurl -lfmt -o test && sudo ./test && cat result.json
...
int main(int argc, char *argv[]){
	...
	if(test_type == "header_test"){
		result_json = test_multi_funcs(tests_json);
	}else if(test_type == "main_test"){
		result_json = test_main(tests_json, 0);
	}else if(test_type == "header_main_test"){
		result_json = test_main(tests_json, 1);
	}
...
// 127 + n (n >= 1 & n <= 31) - fatal error signal:
//     SIGHUP           1
//     SIGINT           2
//     SIGQUIT          3
//     SIGILL           4
