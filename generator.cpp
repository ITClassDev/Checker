...
main_generator::main_generator(json test, vector<string> headers){
	...
	if(test["types"]["in"] != json::array({"void"})){
		for(long long int i=0; i<test["types"]["in"].size(); i++){
			...
			if(variable.find(' ') != -1){
				...
				string extras = variable.substr(variable.find(' '));
				if(extras.find('*') != -1){
	...
	if(test["types"]["out"].is_string() && test["types"]["out"] == "void"){
        ...
        for(long long int i=0; i<test["types"]["out"].size(); i++){
			...
			variable_index -= 1;
			string variable_name = name_vec[variable_index];

			if(variable_name.find('&') != -1){
				variable_name.erase(variable_name.find('&'), 1);
			}

			string variable_type = type_vec[variable_index];
...
void main_generator::add_function_call(string func_name, vector<string> params){
	code += func_name + "(";
	if(params.size() > 0){
