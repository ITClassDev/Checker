...
main_generator::main_generator(json test, std::vector<std::string> headers){
    ...
    if(test["types"]["in"] != json::array({"void"})){
        for(long long int i=0; i<static_cast<long long int>(test["types"]["in"].size()); i++){
            std::string variable = test["types"]["in"][i];
            std::string type;
            std::string name;

            name = "a" + std::to_string(i);
            if(variable.find(' ') != -1lu){
    ...
    if(test["types"]["out"].is_string() && test["types"]["out"] == "void"){
        ...
        if(out_type.find("vector") != -1lu){
            add_vector(1, out_type, "output");
        }else if(out_type == "string"){
            add_string(1, "output");
        }else if(out_type.find("double") != -1lu || out_type.find("float") != -1lu){
...
void main_generator::add_includes(std::vector<std::string> headers){
    for(long long int i=0; i<static_cast<long long int>(headers.size()); i++){
        code = "#include \"" + headers[i] + "\"\n" + code;
    }
    code = "#include <iostream>\n" + code;
    for(long long int i=0; i<static_cast<long long int>(includes.size()); i++){
...
void main_generator::add_function_call(std::string func_name, std::vector<std::string> params){
    code += func_name + "(";
    if(params.size() > 0){
        for(long long int input_i=0; input_i<static_cast<long long int>(params.size()-1); input_i++){
            code += params[input_i] + ",";
        }
