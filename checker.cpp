...
std::string CppChecker::compileTest(std::string unique_id, bool multi){
    ...
    if(platform == "alpine"){
        ...
        wait_for_container(container_id)["StatusCode"]; 
...
json CppChecker::testFunc(json tests, bool debug){
    ...
    func_verdict["solved"] = (static_cast<int>(tests["tests"].size()) == passed_counter);
...
json CppChecker::testMultiFunc(json tests, bool debug){
    ...
    // get first submit id to get all headers
    std::string first_submit_id = "";
    if(tests.size() != 0){ first_submit_id = tests[0]["submit_id"]; }
    else{ return json{{"error", 5}, {"error_msg", ""}}; }

    // get all headers in dir to include
    if(static_cast<int>(headers.size()) != 0) headers.clear();
    for(const auto &file : std::filesystem::directory_iterator(workspace_path + first_submit_id + "/")){
        if(file.path().extension() == ".h"){
            headers.push_back(file.path().filename().string());
        }
    }
...
json PythonChecker::testMain(json tests, bool debug){
    ...
    checker_verdict["solved"] = (static_cast<int>(tests["tests"].size()) == passed_counter);
