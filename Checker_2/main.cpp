...
string cpp_cli_run(string submission_id, string test, json env)
{
	...
	int mem_limit = 6;
	int proc_limit = 2;
	if (env["time"] > time_limit)
	{
		time_limit = env["time"];
	}
	if (env["mem"] > mem_limit)
	{
		mem_limit = env["mem"];
	}
	if (env["proc"] > proc_limit)
	{
		proc_limit = env["proc"];
	}
...
json cpp_test_one_func(json tests, vector<string> headers)
{
	...
	for (json::iterator test_it = tests["tests"].begin(); test_it != tests["tests"].end(); ++test_it)
	{
		...
		if (!status_code)
		{
			...
			passed_counter += output == expected_output;
			if (debug)
				test_verdict.push_back({"debug", (output + " : " + expected_output)});
		}
		else
		{
	...
	final_payload["tests_results"] = func_verdict;
	final_payload["submit_id"] = submission_id;
	final_payload["solved"] = tests["tests"].size() == passed_counter;
	...
	CROW_ROUTE(app, "/homework")
		.methods("POST"_method)([](const crow::request &req)
				...
				if (language == "cpp")
				{
					...
					{
						checker_output = cpp_test_multi_funcs(tests_json);
					}
					else if (test_type == "main_test")
					{
						checker_output = cpp_test_main(tests_json, 0);
					}
					else if (test_type == "header_main_test")
					{
						checker_output = cpp_test_main(tests_json, 1);
					}
				}
				else if (language == "python")
				{
					...
					{
						checker_output = python_test_main(tests_json);
					}
				} 
				return crow::response{checker_output.dump()};
				});
